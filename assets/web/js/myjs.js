  ...
  listB += '<tr>'+
	...
	function tampiltawaran(id){
	   ...
	   // var flag = 0;
    	...
    	html += '<div class="card card-body bg-dark" style="margin-bottom: 10px;"><div class="row"><div class="col-xs-4 pad1 pl-2">'+
                    '<img style="height : 60px; width : 60px;" src="'+window.base_url+'assets/images/foto/'+datat[i].foto+'" alt="">'+
                    // '<img style="height : 60px; width : 60px;" src="'+window.base_url+'assets/images/register.png" alt="">'+
                '</div>'+
                '<div class="col-xs-8 pad1 pl-3">'+
                    '<div><a href="#" class="linkdetail"> '+datat[i].unama+'</a><br /></div>'+
                    '<div class="text-white">Rp '+formatNumber(datat[i].total)+'</div>'+
                '</div>'+
                    ...
                    '<tbody>';
                        ...
                        for(var k=0; k < datap.length*parseInt(jmltaw); k++){
                            if(datap.length>k){
                                ...
                                // html += '<th>Rp. '+formatNumber(datap[flag].jumlah)+'</th>';
                                html += '</tr>';
                                skip++;
                            }
                            flag++;
    ...
    function tampilpengajuan(id){
        ...
        if(datap.length>0){
    	   	for(var i = 0; i < totdata; i++){
    	   	   // var hs = parseFloat(datap[i].harga) / parseFloat(datap[i].jumlah);
    	   	    var hs = parseFloat(datap[i].harga);
    	   	    totalhs += hs;
    	   	    if(tawaranku!==0){
    	   	        if(datap[i].idp==tawaranku[i].pid){
    	   	            htaw=formatNumber(tawaranku[i].hargap); 
    	   	            tothtaw += parseFloat(tawaranku[i].hargap);
    	   	        }
    	   	    }
    	   	    html += '<tr><th>'+(i+1)+'</th><th>'+datap[i].nama+'</th><th class="text-right">Rp. '+formatNumber(hs)+'</th><th class="text-right"><input type="hidden" name="idp['+i+']" value="'+datap[i].idp+'"/>Rp. <input type="text" oninput="hitungTotTaw('+totdata+', this.id)" value="'+htaw+'" name="htawaran['+i+']" id="htawaran'+i+'" style="width:99px; direction:rtl;"></th></tr>';
    	   	}
    	   	html += '<tr><th></th><th>Total: </th><th class="text-right">Rp. '+formatNumber(totalhs)+'</th><th class="text-right" id="tottaw">Rp. '+formatNumber(tothtaw)+'</th></tr>';
        }
    ...
    function hitungTotTaw(totdata, id){
        ...
        var total = 0;
        for(var i = 0; i < totdata; i++){
            total += parseFloat($('#htawaran'+i).val().replace(/,/g, ''));
        }
        if(!isNaN(total)){
            $('#tottaw').html('Rp. '+formatNumber(total));
            $('#totaltaw').html(total);
        }
    }
