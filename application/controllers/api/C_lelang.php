...
class C_lelang extends CI_Controller {
    ...
    public function lelang_buat(){
        ...
        if($secret_key==secret_key){
            ...
            $insert_id=$this->m_lelang->insertlelang($lelang_data);
            if(isset($insert_id)){
                $pekerjaan_req=array();
                ...
                $result=$this->m_pekerjaan->insertPekerjaanBatch($pekerjaan_req);
                if($result==berhasil_buat_pekerjaan){
                    $this->response(null,false,berhasil_buat_lelang);
                }else{
                ...
                foreach($lelang_data_ as $data){
                    ...
                    if($diff<=0){
                        ...
                        if($data->lelang_status!=5 && $data->lelang_status==3){
                            $count++;
                            $this->m_lelang->updatelelang($data->lelang_id,$lelang);
                            $this->m_pekerjaan->updatePekerjaanbyLelangid($data->lelang_id,$pekerjaan);
                        }
                    }
                }
