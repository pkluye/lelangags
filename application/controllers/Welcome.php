...
// 	}
	...
	public function index(){
        if (!$this->session->userdata('data_user')){
            $data["cekbuttontawar"] = false;
        } else {
            $data["cekbuttontawar"] = true;
        }
    ...
    public function getDaftarTawaran($id){
        array_push($id, array('twpekerjaan_status'=>status_enable));
        $dataP = $this->m_crud->selectOrderBy('Lihat_Tawaran_Produk', $id, 'tawaran_anggaran', 'ASC');
        $tawaran = array();
        foreach($dataP as $val){
