...
// 		$this->load->model('m_kategori');
	...
	public function proses(){
	    if (!$this->session->userdata('data_user')){
            ...
            if(!$data_u['user_status']){
                ...
                $datal['lelang_tglselesai'] = $this->input->post('tglselesai');
        ...
        // 		$config['min_height']           = 100;
                ...
                $pekerjaan = $this->m_crud->saveBatch('lelangags_lelang', $datap);
        ...
        if (!$this->session->userdata('data_user')){
            ...
            if(!$data_u['user_status']){
                ...
                if($id===null){
                    ...
                    $data['lelang'] = $this->m_lelang->selectByProduk($id, $per_page, $from, $data_u['user_id']);
    ...
    public function pilih($lid, $mid){
        ...
        $datal['lelang_status'] = 6;
        
        // $pilih = $this->m_lelang->updatelelang($lid, $datal);
        $pilih = $this->m_crud->update('lelangags_lelang', $lid, $datal);
        if($pilih==berhasil_edit){
            redirect(site_url('/profil/lelang'));
        }
    }
    ...
    public function getDaftarTawaran($where){
        ...
        $tawaran =  $this->m_crud->selectOrderBy('Lihat_Tawaran_Produk', $where, 'tawaran_anggaran', 'ASC');
        $produk = array();
