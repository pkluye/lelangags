	...
	public function index(){
	    if (!$this->session->userdata('data_user')){
            ...
            if($data_u['user_status']==status_enable){
                ...
                $data['gambar'] = $this->m_user->selectUserImageby($id);
        ...
        $data_u = $this->session->userdata('data_user');
            if(!$data_u['user_status']){
                ...
                if($data_u['user_status']==status_enable){
                    ...
                    $data['gambar'] = $this->m_user->selectUserImageby($data_u['user_id']);
                    ...
                    $data['lelang'] = $this->m_lelang->selectLelangBy($per_page, $from, array('lelang_userid'=>$data_u['user_id'], 'lelang_status'=>3));
                    $data['pengerjaan'] = $this->m_lelang->selectLelangBy($per_page, $from, array('lelang_userid'=>$data_u['user_id'], 'lelang_status'=>6));
                    $data['penyerahan'] = $this->m_lelang->selectLelangBy($per_page, $from, array('lelang_userid'=>$data_u['user_id'], 'lelang_status'=>7));
      ...
      if (!$this->session->userdata('data_user')){
        ...
        $data_u = $this->session->userdata('data_user');
            if(!$data_u['user_status']){
                ...
                $per_page = 10000;
            	$from = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
                $data['lelang'] = $this->m_tawaran->selectTawaranBy(array('user_id'=>$data_u['user_id'], 'lelang_status'=>3));
                $data['pengerjaan'] = $this->m_tawaran->selectTawaranBy(array('user_id'=>$data_u['user_id'], 'lelang_status'=>6));
                $data['penyerahan'] = $this->m_tawaran->selectTawaranBy(array('user_id'=>$data_u['user_id'], 'lelang_status'=>7));
                // $data['parent'] = $this->m_kategori->getParent();
        	   // $data['prov'] = $this->m_indonesia->selectProvinsi();
